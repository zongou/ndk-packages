...
PKG_EXTNAME=.tar.xz
PKG_SRCURL=${GNU_MIRROR}/gnu/tar/${PKG_BASENAME}${PKG_EXTNAME}

get_source
cd "${BUILD_DIR}/${PKG_BASENAME}"

# find "${WORK_DIR}/packages/tar/" -name "*.patch" | while read -r file; do
# 	patch -up1 <"${file}"
# done
...
PKG_EXTRA_CONFIGURE_ARGS="gl_cv_struct_dirent_d_ino=yes"
...
PKG_EXTRA_CONFIGURE_ARGS="${PKG_EXTRA_CONFIGURE_ARGS} ac_cv_func_mkfifoat=yes"

case "${TARGET}" in
armv7*)
	# https://android.googlesource.com/platform/bionic/+/master/docs/32-bit-abi.md#is-32_bit-on-lp32-y2038
	PKG_EXTRA_CONFIGURE_ARGS="${PKG_EXTRA_CONFIGURE_ARGS} --disable-year2038"
	;;
esac
